...
class _BarcodeViewState extends State<BarcodeView> {
  ...
  void _downloadBarcode() async {
    try {
      ...
      await ImagePickerService.saveImage(imageFile: file, fileName: '${dateTime.toString()}.png');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Container(
        ...
        width: widget.barcode.format == m_s.BarcodeFormat.qrCode ? 200 : Get.width,
        ...
        child: Stack(
          ...
          children: [
            Screenshot(
              ...
              child: Container(
                decoration: CustomWidgetElements.commonDecoration(color: Colors.white),
                padding: EdgeInsets.all(10),
                child: b_w.BarcodeWidget(
                  data: widget.barcode.rawValue ?? "",
                  barcode: b_w.Barcode.fromType(
                    AppUtils.getBarcodeType(widget.barcode.format) ?? b_w.BarcodeType.QrCode,
                  ),
                  width: widget.barcode.format == m_s.BarcodeFormat.qrCode ? 200 : Get.width,
                ),
              ),
