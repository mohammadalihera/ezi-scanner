...
class BarcodeView extends StatefulWidget {
  ...
  const BarcodeView({super.key, required this.barcode, this.isBar = false});
...
class _BarcodeViewState extends State<BarcodeView> {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Container(
        height: widget.isBar ? 100 : 200,
        width: widget.isBar ? Get.width : 200,
        ...
        child: Stack(
          children: [
            ...
            Positioned(
              left: widget.isBar ? Get.width * 0.22 : 65,
              top: widget.isBar ? 10 : 65,
              child: InkWell(
                onTap: () {
                  _downloadBarcode();
                },
