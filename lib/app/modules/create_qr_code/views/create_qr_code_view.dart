              ...
              child: Column(
                ...
                children: [
                  Container(
                    decoration: CustomWidgetElements.commonDecorationWithShadow(),
                    child: Tabs(
                      selectedTabIdentifier: controller.selectedCreateType.value,
                      tabs: ['Qrcode', 'Barcode'].map((ct) => TabItem(label: ct, identifier: ct)).toList(),
                      onTabChange: (v) {
                  ...
                  AppButton(
                    name: controller.otherTypes.value ? 'Generate QR Code' : 'Generate Barcode',
                    onTap: () {
                      if (controller.otherTypes.value) {
  ...
  Widget _buildBarcodeDataInput() {
    return TextFormField(
      ...
      validator: (value) {
        ...
        if (value.length < controller.specs.value.minLength || value.length > controller.specs.value.maxLength) {
        ...
        if (!BarcodeValidator.isValidBarcode(value, controller.selectedType.value)) {
      ...
      children: [
        ...
        Padding(
          ...
          child: DropdownButtonFormField<String>(
            ...
            validator: (value) => value == null || value.isEmpty ? 'Encryption Type cannot be empty' : null,
          ),
        ),
      ],
    );
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          focusedBorder: OutlineInputBorder(
            borderSide: BorderSide(color: Colors.blue, width: 2.0), // Focused state border
          ),
          enabledBorder: OutlineInputBorder(
            borderSide: BorderSide(color: Colors.grey, width: 1.0), // Default state border
          ),
          errorBorder: OutlineInputBorder(
            borderSide: BorderSide(color: Colors.red, width: 1.5), // Error state border
          ),
        ),
      ...
      children: [
        GeneratedCodeView(
          barcodeWidget: BarcodeWidget(
            barcode: GenerateService().getBarcodeFormat(controller.selectedType.value),
            data: controller.barcodeData.value,
            drawText: true,
            width: 200,
            height: 200,
            errorBuilder: (context, error) => const Center(child: Text("Invalid input")),
          ),
